(()=>{
  ...
  function mistakeCard({q,attempt}){
    ...
    return `<article class="year-miss"><div class="year-miss-head"><span class="badge">問${esc(q.questionNo||'—')}</span><strong>${esc(q.title||'')}</strong></div>${chosen?`<div class="year-answer-line"><span>今回</span><b>${esc(chosen)}</b></div>`:''}${correct?`<div class="year-answer-line correct"><span>${essay?'判定':'正答'}</span><b>${esc(correct)}</b></div>`:''}<div class="year-point-title">復習ポイント</div>${points.length?`<ul>${points.map(p=>`<li>${esc(p)}</li>`).join('')}</ul>`:fallback}</article>`;
  }
