(()=>{
  ...
  const NO_FIGURE=new Set(['r5-q1','r7-q3']);
  const cache=new Map();
  ...
  function assetSpec(info){
    const source=SOURCES[info.year];
    if(!source)return null;
    const base=`${source.code}-q${info.q}`;
    if(NO_FIGURE.has(base))return null;
  ...
  async function loadAsset(spec){
    ...
    if(!b64.startsWith('iVBOR'))throw new Error('PNGデータ形式を確認できません');
  ...
  function openModal(src,label){
    ...
    m.querySelector('[data-in]').onclick=()=>{z=Math.min(3,z+.25);apply()};
  ...
  function installGrid(card,info){
    ...
    prompt.parentNode.insertBefore(grid,prompt);left.appendChild(prompt);grid.append(left,panel);
  ...
  async function renderPanel(){
    ...
    const panel=grid.querySelector('.figure-panel');const pages=source.pages?.[info.q]||[];const spec=assetSpec(info);
    const diag=`判定: ${source.code}-q${info.q}${info.id?` / ${info.id}`:''}`;
    if(!spec){
      if(panel.dataset.state==='none')return;panel.dataset.state='none';
      panel.innerHTML=`<h3>原問題資料</h3><p class="fig-note">この問題には切り出し対象の図表がありません。</p><div class="figure-actions">${pages.map(p=>`<a class="figure-open" target="_blank" rel="noopener noreferrer" href="${pageUrl(source.url,p)}">原問題 p.${p}</a>`).join('')}</div><div class="figure-diag">${diag}</div>`;return;
    }
    const specKey=Array.isArray(spec)?spec.join('|'):spec;
    if(panel.dataset.asset===specKey&&(panel.dataset.state==='loading'||panel.dataset.state==='done'))return;
    panel.dataset.asset=specKey;panel.dataset.state='loading';
    panel.innerHTML=`<h3>原問題の図・表</h3><div class="figure-wait">図表を読み込んでいます…</div><div class="figure-diag">${diag} → ${specKey}</div>`;
    try{
      const src=await loadAsset(spec);if(panel.dataset.asset!==specKey)return;
      const label=`${info.year} 機械安全 問${info.q} 原問題の図・表`;panel.dataset.state='done';
      panel.innerHTML=`<h3>原問題の図・表</h3><p class="fig-note">問題資料から図・表だけを切り出しています。問題文と同時に確認できます。</p><div class="figure-image-wrap"><img class="figure-image" src="${src}" alt="${label}"></div><div class="figure-actions"><button class="figure-open" data-enlarge>大きく表示</button><a class="figure-open" target="_blank" rel="noopener noreferrer" href="${pageUrl(source.url,pages[0]||1)}">原問題PDF</a></div><div class="figure-diag">${diag} → ${specKey}</div>`;
      panel.querySelector('img').onclick=()=>openModal(src,label);panel.querySelector('[data-enlarge]').onclick=()=>openModal(src,label);
    }catch(e){
      if(panel.dataset.asset!==specKey)return;panel.dataset.state='error';
      panel.innerHTML=`<h3>原問題の図・表</h3><div class="figure-error">図表を読み込めませんでした。${String(e.message||e)}</div><div class="figure-actions">${pages.map(p=>`<a class="figure-open" target="_blank" rel="noopener noreferrer" href="${pageUrl(source.url,p)}">原問題 p.${p}</a>`).join('')}</div><div class="figure-diag">${diag} → ${specKey}</div>`;
    }
  }
  ...
  addEventListener('DOMContentLoaded',queue);queue();
})();
